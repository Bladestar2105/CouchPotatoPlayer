...
with open('context/IPTVContext.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# 2. Rebuild the whole IPTVProvider component body for safety and reordering.
provider_match = re.search(r"export const IPTVProvider: React\.FC<\{ children: React\.ReactNode \}> = \(\{ children \}\) => \{", content)
provider_start = provider_match.end()

provider_end_match = re.search(r"  return \(", content)
# We want to replace everything from provider_start to provider_end_match.start()
