...
import { findCurrentProgram, findCurrentProgramIndex, findNextProgramIndex } from '../epgUtils';
...
describe('epgUtils', () => {
  ...
  describe('findNextProgramIndex', () => {
    test('should return -1 for empty epg', () => {
      expect(findNextProgramIndex([], new Date())).toBe(-1);
    });

    test('should return index of first program starting after time (in a gap)', () => {
      const time = new Date('2023-10-27T12:30:00Z');
      expect(findNextProgramIndex(mockEpg, time)).toBe(2);
    });

    test('should return index of next program when currently in a program', () => {
      const time = new Date('2023-10-27T10:30:00Z');
      expect(findNextProgramIndex(mockEpg, time)).toBe(1);
    });

    test('should return index of first program when time is before all programs', () => {
      const time = new Date('2023-10-27T09:00:00Z');
      expect(findNextProgramIndex(mockEpg, time)).toBe(0);
    });
    ...
    test('should return index of next program when at exact end of current', () => {
      const time = new Date('2023-10-27T11:00:00Z');
      // At 11:00, Program 2 (idx 1) starts. Array.find(p => p.start > 11:00) will skip it and find Program 3 (idx 2).
      expect(findNextProgramIndex(mockEpg, time)).toBe(2);
    });
  });
