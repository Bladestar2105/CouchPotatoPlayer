 ...
 */
const formatXtreamStart = (date: Date | number): string => {
...
export const generateCatchupUrl = (
  ...
  const endTimeMs = endTime instanceof Date ? endTime.getTime() : endTime;

  const startUnix = Math.floor(startTimeMs / 1000);
  const endUnix = Math.floor(endTimeMs / 1000);
...
export const isProgramCatchupAvailable = (
  channel: Channel,
  programStart: Date | number,
  programEnd: Date | number
): boolean => {
  ...
  const catchupStartMs = nowMs - catchupDays * 24 * 60 * 60 * 1000;

  const programStartMs = programStart instanceof Date ? programStart.getTime() : programStart;
  const programEndMs = programEnd instanceof Date ? programEnd.getTime() : programEnd;

  // Program must have ended and be within catchup window
  return programEndMs <= nowMs && programStartMs >= catchupStartMs;
};
