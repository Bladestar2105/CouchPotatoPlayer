...
const SeriesList = forwardRef<ContentRef, { onReturnToSidebar?: () => void }>((_props, ref) => {
  ...
  // Override the legacy primary with the live accent so poster focus borders,
  // category-bar selection and pull-to-refresh tint follow the user's accent.
  ...
  const selectedSeriesForPreview = useMemo(() => {
    if (selectedSeries.length === 0) return null;
    if (focusedSeriesKey) {
      const focusedSeries = selectedSeries.find((entry) => getSeriesPosterKey(entry) === focusedSeriesKey);
      if (focusedSeries) return focusedSeries;
    }
    if (selectedSeriesKey) {
      const pickedSeries = selectedSeries.find((entry) => getSeriesPosterKey(entry) === selectedSeriesKey);
      if (pickedSeries) return pickedSeries;
    }
    return selectedSeries[0];
  }, [focusedSeriesKey, selectedSeriesKey, selectedSeries]);
  ...
  useEffect(() => {
    ...
    prefetchSeriesBatch(selectedSeries.slice(0, 24));
      ...
      <View style={[styles.mainContent, isMobile ? { flex: 1 } : { backgroundColor: colors.background }]}>
        ...
        {selectedSeries.length > 0 ? (
          <View style={styles.tvContentRow}>
            <View style={[styles.gridColumn, Platform.isTV && { borderRightWidth: 1, borderRightColor: colors.divider }]}>
              <FlatList
                ...
                onViewableItemsChanged={onSeriesViewableItemsChanged}
                viewabilityConfig={seriesViewabilityConfig}
                renderItem={renderSeriesItem}
              />
            </View>
            {Platform.isTV && (
              <View style={[styles.previewPanel, { backgroundColor: colors.surface }]}>
                <Text style={[styles.previewLabel, { color: colors.textMuted }]}>{t('tv.selectedSeries')}</Text>
                <Text style={[styles.previewTitle, { color: colors.text }]} numberOfLines={3}>
                  {selectedSeriesForPreview?.name || '—'}
                </Text>
          ...
          <View style={styles.centeredContainer}>
            <Text style={{ color: colors.textMuted }}>{t('tv.noSeriesAvailable')}</Text>
          </View>
        )}
      </View>
      )}
