...
export const StreamHealthMonitor = ({
  ...
  const latencyText = latency === null ? '...' : latency < 0 ? t('error') : `${latency}ms`;

  const urlObj = streamUrl ? new URL(streamUrl) : null;
  const protocol = urlObj?.protocol.replace(':', '') || 'http';
  const host = urlObj?.host || t('streamHealth.unknownHost');

  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Activity color="#4CD964" size={18} />
        <Text style={styles.title}>{t('streamHealth.title')}</Text>
      </View>

      <View style={styles.grid}>
        <View style={styles.cell}>
          <Text style={styles.label}>{t('streamHealth.resolution')}</Text>
        ...
        <View style={styles.cell}>
          <Text style={styles.label}>{t('streamHealth.bitrate')}</Text>
          <View style={styles.valueRow}>
        ...
        <View style={styles.cell}>
          <Text style={styles.label}>{t('streamHealth.latency')}</Text>
          <View style={styles.valueRow}>
        ...
        <View style={styles.cell}>
          <Text style={styles.label}>{t('streamHealth.protocol')}</Text>
          <View style={styles.valueRow}>
        ...
        <View style={styles.cell}>
          <Text style={styles.label}>{t('streamHealth.server')}</Text>
          <View style={styles.valueRow}>
      ...
      {bufferHealth.isBuffering && (
        <View style={styles.bufferingWarning}>
          ...
          <Text style={styles.bufferingText}>{t('streamHealth.buffering')}</Text>
        </View>
      )}
    </View>
  );
